...
const styles = StyleSheet.create({
  ...
  logo: { width: 48, height: 48, marginBottom: 6, alignSelf: 'center', objectFit: 'contain' },
...
function ReportCardInner() {
  ...
  useEffect(() => {
    ...
    Promise.all([listClasses(profile.schoolId), listExams(profile.schoolId), listStudents(profile.schoolId), getSchool(profile.schoolId)]).then(
      ([c, ex, allStudents, school]) => {
        setClasses(c);
        setExams(ex);
