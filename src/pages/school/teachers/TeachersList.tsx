...
function TeachersListInner() {
  ...
  async function handleInvite(t: Teacher) {
    const email = window.prompt(`Send a login invite to which email for ${t.fullName}?`, t.email ?? '');
    if (!email) return;
    setInviteMsg(null);
    try {
      const result = await inviteTeacherLogin(t.id, email);
      setInviteMsg(result.message);
      load(search);
    } catch (err) {
      setErrorMsg(getErrorMessage(err, 'Failed to invite teacher login.'));
    }
  }
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-3">
                      <PermissionGate code="teachers.edit">
                        {!t.userId && (
                          <button onClick={() => handleInvite(t)} className="text-sm text-primary-600 hover:underline">
                            Invite login
                          </button>
                        )}
                        <Link
