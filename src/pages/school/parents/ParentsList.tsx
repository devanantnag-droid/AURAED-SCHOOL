...
function ParentsListInner() {
  ...
  async function handleCreateLogin(email: string, password: string) {
    ...
    setInviteMsg(null);
    try {
      const result = await invitePortalLogin('parent', inviteTarget.id, email, password);
      setInviteMsg(result.message);
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-3">
                      <PermissionGate code="parents.edit">
                        {!p.userId && (
                          <button onClick={() => setInviteTarget(p)} className="text-sm text-primary-600 hover:underline">
                            Create portal login
                          </button>
                        )}
                        {p.userId && (
                          <button onClick={() => setResetTarget(p)} className="text-sm text-primary-600 hover:underline">
                            Reset password
                          </button>
                        )}
                      </PermissionGate>
