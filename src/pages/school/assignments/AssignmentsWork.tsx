...
function AssignmentsInner() {
  ...
  async function handleDownload(bucket: 'assignments', path: string) {
    try {
  ...
  async function toggleExpand(a: Assignment) {
    ...
    try {
      if (profile?.schoolId) {
        await ensureSubmissionRows(profile.schoolId, a.id, a.classId, a.sectionId);
      }
      setSubmissions(await listSubmissions(a.id));
      setExpandedId(a.id);
    } catch (err) {
      setErrorMsg(getErrorMessage(err, 'Failed to load submissions.'));
    }
  }
  ...
  async function handleGrade(submissionId: string) {
    ...
    try {
      ...
      if (expandedId) setSubmissions(await listSubmissions(expandedId));
    } catch (err) {
      setErrorMsg(getErrorMessage(err, 'Failed to save grade.'));
    }
  }

  return (
    <div className="mx-auto max-w-3xl p-6">
      <h1 className="mb-4 text-xl font-semibold text-gray-900 dark:text-gray-50">Assignments</h1>
      <p className="mb-4 text-xs text-gray-500">
        Note: students don't have self-submission accounts yet, so grading starts from the class roster —
        enter marks directly for each student below.
      </p>
              ...
              <li key={a.id} className="rounded-md border border-gray-200 text-sm dark:border-gray-800">
                <div className="flex items-center justify-between p-3">
                  ...
                  <div className="flex items-center gap-3">
                    {a.attachmentPath && (
                      <button onClick={() => handleDownload('assignments', a.attachmentPath!)} className="flex items-center gap-1 text-xs text-primary-600 hover:underline">
                        <Paperclip size={12} /> Attachment
                      </button>
                    )}
                    <PermissionGate code="assignments.grade">
                      <button onClick={() => toggleExpand(a)} className="flex items-center gap-1 text-xs text-gray-600 hover:underline dark:text-gray-400">
                        Grade {expandedId === a.id ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                      </button>
                    </PermissionGate>
                        ...
                        {submissions.map((s) => (
                          <li key={s.id} className="flex items-center justify-between gap-2 text-xs">
                            ...
                            <button
                              ...
                              className="rounded-md bg-primary-600 px-2 py-1 text-white hover:bg-primary-700"
                            >
                              Save
                            </button>
                          </li>
